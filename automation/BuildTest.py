...
if not (os.path.exists(configFilePath)):
    print(ERROR_COLOR + "\nError: config.json not found! Exiting with status code: 1" + NORMAL_COLOR)
    exit(1)
...
if (not ("paths" in config) or not (config["paths"])) \
    ...
    print(ERROR_COLOR + "config.json is poorly formatted, or missing values required for script! Exiting with status code: 2" + NORMAL_COLOR)
    exit (2)
...
if (result != 0):
    print(ERROR_COLOR + "Error applying DPE. See output for details. Exiting with status code: 3" + NORMAL_COLOR)
    exit(3)
...
if (result != 0):
    print(ERROR_COLOR + "Error applying additional patch files. See output for details. Exiting with status code: 5" + NORMAL_COLOR)
    exit(4)
...
result = call(["python", "./scripts/make.py"])
if (result != 0):
    print(ERROR_COLOR + "Error applying CFRU. See output for details. Exiting with status code: 6" + NORMAL_COLOR)
    exit(5)

print(SUCCESS_COLOR + "Success! Launching built Test.gba. Happy Testing :)" + NORMAL_COLOR)
