...
As an example if you imported `gEventObjectPic_JohnDoe.png` you'll want to get a reference to the palette by adding `extern const u8 gEventObjectPic_JohnDoePal[];` to `src\character_customization.c`. 

Then you will need to add that reference to a palette table with a unique palette tag. Let's say we want to use palette tag `0x1201`. Find the table associated with the high byte (`0x12` in this case), there's already a table set up `sObjectEventSpritePalettes12`. 

Add an entry for the palette to the table associating it with the chosen ID `{gEventObjectPic_JohnDoePal, 0x1201}` before the NULL entry. 

At this point a sprite pointing to palette tag `0x1201` should use your palette!
