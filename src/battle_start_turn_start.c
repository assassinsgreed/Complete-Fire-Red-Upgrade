...
bool8 TryActivateOWTerrain(void)
{
	...
	else if (MAP_IS(ORICHELLE_GARDEN))
	{
		...
		MAP_IS(UTEYAN_RUINS_CAVE_1F) ||
		MAP_IS(UTEYAN_RUINS_CAVE_2F) ||
		...
		else
		{
			...
			if (trainerClass == CLASS_GYM_TRAINEE)
			{
				switch gTrainerBattleOpponent_A // Trainer ID
				{
					...
					case TRAINER_TSARVOSA_CITY_GYM_TRAINEE_DARCY:
					...
					case TRAINER_TSARVOSA_CITY_GYM_TRAINEE_CASPAR:
						return BGM_BATTLE_SINNOH_LEADER;
					default:
						return BGM_BATTLE_GYM_LEADER; // Only ever hit by Faye and unmapped trainers
				} 
			}
